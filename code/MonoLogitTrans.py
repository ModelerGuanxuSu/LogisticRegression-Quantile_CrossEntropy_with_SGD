...
strict_check : boolean, default: False
    If set to be True, then the algorithm would try to pretend the transformed value
    from being a constend because of the constraint of monotone, by removing a percentage
    of largest value of the origin and the corresponding elements of Y.
...
load_parameter: load the parameters from a pickle file
    ...
    def __init__(self, max_iter=150, batch_size=1024, num_hidden=10, lambda_monotonous=1, method='wide',
                 target='classifier', strict_check=False):
    ...
    def fit(self, dataX, dataY):
        ...
        def method_trans(self, dataX, dataY):
            if self.method == 'wide':
    ...
    def save_parameter(self, path):
        with open(path, 'wb') as f:
            pickle.dump(
                [self.weights, self.biases, self.x_mean, self.x_std, self.relation, self.x_mean_data, self.x_std_data,
                 self.x_max_data, self.x_min_data], f)


    def load_parameter(self, path):
        with open(path, 'rb') as f:
            self.weights, self.biases, self.x_mean, self.x_std, self.relation, self.x_mean_data, self.x_std_data, self.x_max_data, self.x_min_data = pickle.load(
                f, encoding='latinl')
...
def fit_network(dataX_, dataY_, num_hidden=10, lambda_monotonous=2, max_iter=150, batch_size=1024, target='classifier'):
    ...
    if target == 'classifier':
        loss_y = tf.reduce_mean(-y * tf.log(tf.clip_by_value(x_trans, 1e-10, 1.0 - 1e-10)) - (1 - y) * tf.log(
            tf.clip_by_value(1 - x_trans, 1e-10, 1.0 - 1e-10)))
    elif target == 'regression':
        loss_y = tf.losses.mean_squared_error(y, x_trans_linear)
    ###单调限制网络
    w1_1 = tf.Variable(tf.random_normal([1], stddev=0.1, mean=1), name='w1_1', dtype=tf.float32,
                       constraint=make_positive)
    w1_3 = tf.Variable(tf.random_normal([1], stddev=0.1, mean=1), name='w1_3', dtype=tf.float32,
                       constraint=make_positive)
    w1_5 = tf.Variable(tf.random_normal([1], stddev=0.1, mean=1), name='w1_5', dtype=tf.float32,
                       constraint=make_positive)
    w1_exp = tf.Variable(tf.random_normal([1], stddev=0.1, mean=1), name='w1_exp', dtype=tf.float32,
                         constraint=make_positive)
    ...
    loss_monotonous = 1 - tf.abs(
        x_mono_trans * x_trans_linear_centre / (tf.sqrt(tf.reduce_sum(x_mono_trans * x_mono_trans)) * \
                                                tf.sqrt(tf.reduce_sum(
                                                    x_trans_linear_centre * x_trans_linear_centre))))  # cosine similirity/Pearson correlation
    ...
    step_ = 2
    while (((Loss[-2] - Loss[-1]) / (np.abs(Loss[-2]) + 1e-10) > .001) or ((Loss[-2] - Loss[-1]) < 0) or step_ < 30) & (
            step_ <= max_iter):
        step_ += 1
        index_batch = [random.choices(range(dataY.shape[0]))[0] for i in range(batch_size)]
        batch_X = dataX[index_batch]
...
def fit_network_strict_mono(dataX_, dataY_, num_hidden=10, lambda_monotonous=2, max_iter=150, batch_size=1024,
                            target='classifier'):
    '''
    The optimazation would stop if loss function reduces less than 0.01%, or number of iterations reaches
    max_iter.
    ...
    ###主网络
    with tf.variable_scope('weight', initializer=tf.random_normal_initializer(), regularizer=regularizer):
        w1_pos = tf.Variable(tf.random_normal([1, num_hidden], stddev=0.1, mean=0.5), name='w1_pos', dtype=tf.float32,
                             constraint=make_greater_than_1)
        w2_pos = tf.Variable(tf.random_normal([num_hidden, num_hidden], stddev=0.1, mean=0.5), name='w2_pos',
                             dtype=tf.float32, constraint=make_positive)
        w3_pos = tf.Variable(tf.random_normal([num_hidden, 1], stddev=0.1, mean=0.5), name='w3_pos', dtype=tf.float32,
                             constraint=make_greater_than_1)
    ...
    if target == 'classifier':
        loss = tf.reduce_mean(-y * tf.log(tf.clip_by_value(x_trans, 1e-10, 1.0 - 1e-10)) - (1 - y) * tf.log(
            tf.clip_by_value(1 - x_trans, 1e-10, 1.0 - 1e-10)))
    elif target == 'regression':
...
def get_data():
    ...
    x1, x2, x3, x1_test, x2_test, x3_test, y, y_test = X_train[:, index1], X_train[:, index2], X_train[:, index3], \
                                                       X_test[:, index1], X_test[:, index2], X_test[:,
                                                                                             index3], Y_train, Y_test
...
if __name__ == '__main__':
    ...
    def linear_(x1, x2, x3):
        x2_ = x2
        x2_[x2_ > np.quantile(x2_, 0.7)] = np.random.uniform(min(x2_), np.quantile(x2_, 0.5),
                                                             size=[sum(x2_ > np.quantile(x2_, 0.7))])
        return (((np.exp(1) ** x1 / 20 - 20 * x1 ** 2 - 0.1 * x1 ** 3) / 20 - np.exp(
            x2_) / 10 + x2_ ** 2 + x3 * 2 + np.random.normal(-10, 10, size=[len(x1)])))
